 ...
 *
 * All credit for this library to Professor Luther Tychonievich
 *
 *
 ...
 * so use the provided image_xy macro (C) or the Image wrapper class (C++) for
 * access.
 */
typedef struct {
 ...
 * if (img == NULL) { fprintf(stderr, "%s was not a PNG file",
 * "existing_image.png"); } for(int y = 0; y < img->height; y += 1) { for(int x
 * = 0; x < img->width; x += 1) { pixel_t pixel = pixel_xy(img, x, y);
 *   }
 * }
 * ~~~~
 */
...
public:
  ...
  /// array access to image. Use as `img[y][x].red` or the like. Note the
  /// y-first order!
  pixel_t* operator[](uint32_t y) { return &(data->rgba[data->width * y]); }

  /// store the image in a PNG file
  void save(const char* filename) const { save_image(data, filename); }
